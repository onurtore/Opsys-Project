...
asmlinkage void schedule(void)
{
	...
	extern int option;  // CSE OPSYS PROJECT
...
repeat_schedule:
	...
	if(option == 0) {  // CSE OPSYS PROJECT - START
		c = -1000;		
	...
	else if(option == 1){ // CSE OPSYS PROJECT	
		int A = 7;
		int B = 1;
		int C = 40;
		int D = 120;
		list_for_each(tmp, &runqueue_head) {
			p = list_entry(tmp, struct task_struct, run_list);
			if (can_schedule(p, this_cpu)) {
				unsigned long runtime = (jiffies - (p->last_run))*10;
				if(runtime < C){
					if(p->ticket_number > B){
						p->ticket_number--;
					}
				}
				else if(runtime > D){
					if(p->ticket_number < A){
						p->ticket_number++;
					}
				}
...
backtoloop:
		...
		list_for_each(tmp, &runqueue_head) {
			...
			if (can_schedule(p, this_cpu)) {
				if(p->used == 0){
					rand = rand - (p->ticket_number);
					if(rand <= 0){
						next = p;
						p->last_run = jiffies;
						detected_user = p->user->uid;
						break;
					}
				}
			}
		...
		list_for_each(tmp, &runqueue_head) {
			p = list_entry(tmp, struct task_struct, run_list);
			if (can_schedule(p, this_cpu)) {
				if(p->user->uid == detected_user){
					p->used = 1;
				}
			}
		}
		
	}// CSE OPSYS PROJECT - END
